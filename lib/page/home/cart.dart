...
class _CartViewState extends State<CartView> {
  ...
  _fillInOrder() {
    ...
    var unCheckedList = _cartList.where((c) => c.checked == false).toList();
    if (unCheckedList.length == _cartList.length) {
      ToastUtil.showToast("请勾选商品");
      return;
    }
      ...
      child: InkWell(
        ...
        child: Card(
          child: Row(
            ...
            children: <Widget>[
              Checkbox(
                  value: _cartList[index].checked ?? false,
                  activeColor: Colors.deepOrangeAccent,
                  onChanged: (bool) {
...
//      cartNumberEventBus.fire(CartNumberEvent(number - 1));
  ...
  _setCartItemCheck(bool checked) async {
    Options options = Options();
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    String nickname = sharedPreferences.getString(Strings.NICK_NAME);
    if (_isAllCheck) {
      await HttpUtil.instance.get(
          Api.DOMAIN+'/index/index/checkedAll?username=${nickname}&status=0',
          options: options);
    } else {
      await HttpUtil.instance.get(
          Api.DOMAIN+'/index/index/checkedAll?username=${nickname}&status=1',
          options: options);
    }
    setState(() {
      _isAllCheck = checked;
