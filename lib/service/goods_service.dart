...
class GoodsService {
  ...
  Future cartCheckOut(OnSuccess onSuccess, OnFail onFail,
      Map<String, dynamic> parameters) async {
    try {
      String _url = Api.DOMAIN+'/index/index/cartList';
      print('~~~~~~~~~');
      print(parameters);
      var response = await HttpUtil.instance.get(_url, parameters: parameters);
      if (response['errno'] == 0) {
  ...
  Future buy(
    ...
    try {
      String _url = Api.DOMAIN+'/index/index/buyGoods';
      print(_url);
      var response = await HttpUtil.instance
          .post(_url, parameters: parameters,);
