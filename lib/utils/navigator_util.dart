...
class NavigatorUtils {
  ...
  static goCart(BuildContext context) {
    Application.router.navigateTo(context, Routers.cart,
        transition: TransitionType.inFromRight);
  }
  static goGoodsEvaluation(BuildContext context,String goodsName,int goodsId,String orderSn, int productId) {
    var good = FluroConvertUtil.fluroCnParamsEncode(goodsName);
    var id = FluroConvertUtil.fluroCnParamsEncode(goodsId.toString());
    var oId = FluroConvertUtil.fluroCnParamsEncode(orderSn);
    var pid = FluroConvertUtil.fluroCnParamsEncode(productId.toString());
    Application.router.navigateTo(context, Routers.goodsEvaluation+"?goodsName=$good&goodsId=$id&orderSn=$oId&productId=$pid",
        transition: TransitionType.inFromRight);
  }
  static goGoodsComments(BuildContext context, int goodsId) {
    Application.router.navigateTo(
        context, Routers.goodsComments + "?goodsId=$goodsId",
        transition: TransitionType.inFromRight);
  }
  static goApplyRefund(BuildContext context,String orderSn) {
    var oId = FluroConvertUtil.fluroCnParamsEncode(orderSn);
    Application.router.navigateTo(context, Routers.applyRefund+"?orderSn=$oId",
        transition: TransitionType.inFromRight);
  }

  static goApplyExpress(BuildContext context,String orderSn) {
    var oId = FluroConvertUtil.fluroCnParamsEncode(orderSn);
    Application.router.navigateTo(context, Routers.applyExpress+"?orderSn=$oId",
        transition: TransitionType.inFromRight);
  }
}
