...
import 'package:mall/page/goods/good_eveluations.dart';
import 'package:mall/page/goods/goods_coments.dart';
import 'package:mall/page/home/cart.dart';
import 'package:mall/page/home/mall.dart';
import 'package:mall/page/mine/apply_refund.dart';
import 'package:mall/page/mine/refund_express.dart';
...
var fillInOrderHandler = Handler(
    handlerFunc: (BuildContext context, Map<String, List<String>> parameters) {
  int cartId = int.parse(parameters["cartId"].first);
...
var cartHandler = Handler(handlerFunc: (context, parameters) {
  return CartView();
});

var goodsEvaluationHandler = Handler(handlerFunc: (context, parameters) {
  print('parameters["productId"].first');
  print( int.parse(FluroConvertUtil.fluroCnParamsDecode(parameters["productId"].first)));
  return GoodsEvaluationView(
    goodsName:
        FluroConvertUtil.fluroCnParamsDecode(parameters["goodsName"].first),
    orderSn: FluroConvertUtil.fluroCnParamsDecode(parameters["orderSn"].first),
    goodsId: int.parse(FluroConvertUtil.fluroCnParamsDecode(parameters["goodsId"].first)),
    productId: int.parse(FluroConvertUtil.fluroCnParamsDecode(parameters["productId"].first))
  );
});

var goodsCommentsHandler = Handler(handlerFunc: (context,parameters){
  var goodsId = int.parse(parameters["goodsId"].first);
  return GoodsCommentsPage(goodId: goodsId,);
});

var applyRefundHandler = Handler(handlerFunc: (context, parameters) {
  return ApplyRefundView(
    orderSn: FluroConvertUtil.fluroCnParamsDecode(parameters["orderSn"].first),)
  ;
});
var applyExpressHandler = Handler(handlerFunc: (context, parameters) {
  return ApplyExpressView(
    orderSn: FluroConvertUtil.fluroCnParamsDecode(parameters["orderSn"].first),)
  ;
});